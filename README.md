...
- 不支持y轴翻转
- 支持选择作用范围
- 若弹幕坐标经处理后小于0的则弹窗提醒，并钳至0，若勾选`允许负值`则不进行此操作
...
- 支持z轴旋转
- 不支持y轴翻转
- 若新建弹幕坐标有小于0的则弹窗提醒，并钳至0，若勾选`允许负值`则不进行此操作
...
- 格式：
  - 命令类型声明`/s`
  - `规则列表`使用`;`分隔，`筛选规则`使用`,`分隔
  - 单个`筛选规则`格式：`筛选字段名`:"`规则`"
  - 应用于同一`筛选规则`中的多条规则：在双引号内使用`,`分隔
    - 数值范围：在双引号内格式为`数值~数值`，其中`数值`可为其他字段参与计算的表达式
    - 严格匹配：在双引号内使用单引号包裹匹配值，如：`'匹配值'`
...
| `\` | 时间分割工具 |
| `/` | 展开高级工具面板 |
| `Ctrl + ArrowDown` | 将起始坐标应用至结束坐标 |
| `Ctrl + ArrowUp` | 将结束坐标应用至起始坐标 |

## 时间轴与播放头

| 快捷键 | 作用 |
| --- | --- |
| `Ctrl + Drag / Click` | 多选弹幕 |
...
7. 做完后想要录屏分享？打开`录屏模式`！（Ctrl + Alt + Space）自动全屏，抛弃所有不必要的渲染，专注于性能，完成录屏后按`ESC`回到编辑模式
