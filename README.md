...
**赋值计算命令**：
- 格式：
  - `被赋值字段名 = 赋值计算式（支持四则，支持其他字段作为变量参与计算）`
  - 支持使用`;`分隔设置多个赋值规则
  - 特殊赋值规则：upset
    - 格式：`被赋值字段名 = upset`，意为在选中弹幕范围内打乱所选字段，此方法不可使用计算
...
通过 `Ctrl + ;` 可唤出高级创建工具，此模块采用“工具面板生成规则 -> 写入预览 JSON -> 用户确认后点击创建”的流程，和主编辑器数据解耦，适合批量造大量弹幕
...
- 所有数值字段都支持三种生成模式：
  - `循环`：按输入列表循环取值，支持表达式，无需开启 `表达式` 即可解析
...
- `text`文本字段支持使用类似js字符串拼接的格式
  - 输入的起始与结束字符必须是反单引号：\`，使用`${}`声明表达式，如：**\`当前：${round(t * 100)}%\`** => 输出例：`当前：25%`
  - 该解析方法兼容`循环列表`模式
...
- `width`: 设置的屏幕宽度
- `height`: 设置的屏幕高度
- **所有**数值字段当前索引的值，使用时直接复制编辑单元下显示的路径作为变量即可，例：`transform.start.x`
...
6. 使用[DMsender_CLI](https://github.com/qmwNEbrVt2233/DMsender_CLI/releases)or[弹幕发射场](https://github.com/MikuFan039/DMSenderAPP/releases)或其他任何用于发送 xml 的工具将你的弹幕上传至 bilibili 吧！
  - 【广告】DMsender_CLI极其轻量，支持断点续传且支持向私有视频发送，有丰富的错误处理方法，甚至可以在发送时修改单条弹幕数据，如果你只想发送弹幕，那么它定是不二之选！
